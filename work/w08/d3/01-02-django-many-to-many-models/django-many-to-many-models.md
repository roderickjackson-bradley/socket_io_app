...
Now that we have One-to-Many relationships under our belt, it's time to see how many-to-many relationships work in Django.
...
To be able to display the list of unassociated toys, we'll need to query for them in the `cats_detail` view function and add them to the context currently being passed to the template.

The query to find all toys that a cat doesn't have is somewhat complicated, but it demonstrates the power of the Django ORM.
