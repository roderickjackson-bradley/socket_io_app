...
> When you deploy your projects next week, we will need to set these very same keys **directly** on Heroku, the application hosting service we will be using.
...
This is because Django templates automatically call an attribute if it's a function. This can be a problem if you ever need to actually call a function that takes arguments.  For example, this **will not work**:
...
For example, to check the length, this would work:
...
Okay, let's add a handmade `<form>` we can use to upload files to the application
...
Other than `{% csrf_token %}`, it's pretty much a generic form that can be used to send a file to any web app.
