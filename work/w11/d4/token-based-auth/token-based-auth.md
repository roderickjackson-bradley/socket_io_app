...
Next, in order to add `user` to the state when the app is loaded or refreshed, we will update the `componentDidMount` lifecycle method.
...
To implements logging in, we need to write code on both the client and server...
