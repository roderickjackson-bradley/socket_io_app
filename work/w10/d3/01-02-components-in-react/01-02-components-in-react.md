...
Let's check out what we're going to build [here](https://wdi-mastermind.herokuapp.com/).
...
Open **App.js** where the `<App>` component is defined as a class component.
...
For now, the only differences you need to be aware of between Function Components and Class Components are:
...
- You access props in a Class Component as `this.props` instead of just `props` - the name of the parameter commonly used when defining Function Components.
...
Let's clean up **App.css** while we're at it. Replace everything with the following:
...
Function components are functionally equivalent to React class components when the class only defines a `render` method.  For example, the equivalent class component for the above `<ConcertCard>` component would be:
...
Next, add the **component** class to the `<div>` in the `<GameBoard>` component like this:
...
- **Will there be more _presentational_ or _container_ type components in a React app?**

- **Can a _presentational_ component be defined using a class?**

- **When would you have to define a component as a class instead of a function?**
